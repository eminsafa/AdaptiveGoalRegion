...
input(
    f"{robot_controller.latest_capture_path}\nWaiting for GPD, Press enter when ready!"
)

matrices_data = robot_controller.read_grasping_poses_file()  # orientation as quaternion
poses = robot_controller.convert_matrices_to_array(
    matrices_data
)  # orientation as quaternion
# poses = robot_controller.array_frame_transformation(poses)  # orientation as quaternion

adaptive_goal_region_data = agr(
    poses, rotate=False, return_euler=False
)  # Includes -45 degree rotation
# save_agr_data_as_txt(
#     adaptive_goal_region_data
# )  # NOTE THAT RESULTS MIGHT BE IN QUATERNION FORMAT, CHECK THE LINE ABOVE
save_agr_data_as_csv(adaptive_goal_region_data, "raw")
# visualize_grasps(adaptive_goal_region_data)

poses = robot_controller.array_frame_transformation(poses)  # orientation as quaternion
adaptive_goal_region_data = agr(
    poses, rotate=True, return_euler=False
)  # Includes -45 degree rotation
# save_agr_data_as_txt(
#     adaptive_goal_region_data
# )  # NOTE THAT RESULTS MIGHT BE IN QUATERNION FORMAT, CHECK THE LINE ABOVE
save_agr_data_as_csv(adaptive_goal_region_data, "converted")
# visualize_grasps(adaptive_goal_region_data)
